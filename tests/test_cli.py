...
def test_cli_validate_solve_sweep_on_every_engine(tmp_path, capsys):
    ...
    for name, path in (("ch3", os.path.join(EX, "ch3_two_player.yaml")), ("ch1", os.path.join(EX, "ch1_two_player_finite.yaml"))):
        out = tmp_path / f"{name}.json"; plot = tmp_path / f"{name}.png"
        assert main(["solve", path, "-o", str(out), "--plot", str(plot)]) == 0
        d = json.load(open(out)); assert d["converged"] and d["kernels"] and d["grid"]["kind"] in ("stationary", "finite")
...
def test_require_ok_covers_the_guards_that_check_does_not(tmp_path, capsys):
    ...
    with pytest.raises(ns.DiagnosticsError, match="converged, but"):
        r.require_ok()
    #  a model whose every required check actually RAN and passed (a check that could not be run at all is
    #  the cell engine's case: extras/test_cells.py)
...
def test_a_weaker_standard_can_be_asked_for_by_name(capsys):
    ...
    standard it insists be NAMED could not be named from the command line at all.  (ch3_two_player at 12
    nodes converges and fails its window guard; EXPLORATORY requires convergence only.)"""
    import noisestate as ns
    argv = ["solve", ns.example("ch3_two_player"), "--nodes", "12", "--require-ok"]
