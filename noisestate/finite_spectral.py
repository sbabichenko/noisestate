...
class SpectralCompiled:
    def __init__(self, model: Model):
        ...
        for a in model.agents:
            atoms: List[Atom] = []; terms = []
            for term in a.loss:
                coef = float(term[0]); ex = [model.expand({s: 1.0}) for s in term[1:]]
                for e in ex:
                    for k in e:
                        if k not in atoms:
                            atoms.append(k)
                terms.append((coef, ex))
            m = len(atoms); Q = np.zeros((m, m)); q = np.zeros(m)
                ...
                if len(ex) == 1:
                    ...
                    for k1, c1 in ex[0].items():
                        for k2, c2 in ex[1].items():
                            ...
                            Q[i, j] += coef * c1 * c2; Q[j, i] += coef * c1 * c2
            self.loss[a.name] = (atoms, Q, q)
        self.rep = {a.name: a.name for a in model.agents}
