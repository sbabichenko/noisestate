...
From the directory holding the dissertation's `kb_spectral.cpp` and `kb_spectral_q.cpp`, with this
repository checked out at `$NOISESTATE`:

    patch -p0 kb_spectral.cpp   < $NOISESTATE/extras/patches/kb_spectral_cascade_total_flow_rows.patch
    patch -p0 kb_spectral_q.cpp < $NOISESTATE/extras/patches/kb_spectral_q_cascade_total_flow_rows.patch
