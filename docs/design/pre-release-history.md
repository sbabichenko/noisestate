...
Archived development notes from the work leading to the first public release, 1.0.0.
This includes the original 1.0.0 preparation notes and the internal 0.x release history.
API names, limitations, and measurements describe the versions discussed, not necessarily
the current package. Unlinked file paths are relative to the repository root.

See the [public changelog](../../CHANGELOG.md) for published releases and
the [validation reference](../validation.md) for numerical validation.

## 1.0.0 (2026-09-10) — first public release

First public release, following the 0.x development series. Bundled examples now
install with the wheel, tests check the documentation against the API, and the
README includes a complete introductory workflow. Solver validation against the
dissertation's computations was established in versions 0.5–0.8; see
[validation](../validation.md). This release adds diagnostic fixes, API
improvements, and documentation updates.

Earlier entries describe behavior at the time of each release. Historical API
names, measurements, and limitations may differ from the current package.


### Transition diagnostic applicability

Fixed three related diagnostic defects identified in the
[newcomer review](newcomer_friction_2026-09-10.txt).

Check applicability depended only on horizon kind. Transitions were assigned
window and settling checks even when those checks did not apply. The absent
records were classified as MISSING, preventing every transition from receiving
publication acceptance.

Applicability now depends on the model's time structure. Stationary models use
the `window` check. Transitions use `past window`, `continuation window`, and
`settled` where applicable. A game ending at T has no settling requirement; an
initial-state prior has no past window. No shipped example now reports MISSING.

`past window` and `continuation window` records were emitted but were absent
from policy definitions. Their failures therefore could not block acceptance.
Both are now included in CHECKS, Policy.PUBLICATION, and MINIMUM. This fix was
needed alongside the applicability correction to prevent acceptance of
transitions whose inherited windows failed their checks.

### Second-order checks at positive discount

The stationary engine incorrectly disabled the second-order check at rho > 0.
The earlier rationale was that the discounted objective was not quadratic in
the stationary kernel. The dissertation gives the discounted objective as

    J = 1/2 E int_0^inf e^{-rho t}
        [X' G^XX X + 2 G^X' X + 2 D' G^DX X + D' G^DD D] dt,   rho > 0

with a positive-semidefinite joint running Hessian. This Hessian is local in
time and contains no rho; discounting supplies the positive weight e^{-rho t}.
A form that is semidefinite at each time retains its sign under that weight.
The check therefore uses the average-cost system. The Kyle-Back chapter uses
the same convention and reports a positive-definite exact quadratic form with
smallest eigenvalue 2 eps.

The stationary cost Gram already used the average-cost form at every rho.
SECOND_ORDER_QUADRATIC is now True, and curvature_is_obtainable has been removed.
At rho = 1e-9, 0.1, 0.5, and 1.0, the measured curvature remained positive and
the verdict was unchanged.

`ch4_kyle_back` now passes its second-order check and receives publication
acceptance. Previously, the disabled check prevented acceptance at any resolution.

The cell engine still reports UNSUPPORTED for representation error and
second-order checks, since it computes neither. The CLI explains this limitation.

### Discount conventions and validation

A follow-up audit found no further arithmetic defect in discount handling.
Stationary first-order conditions apply discounting through the correlation
tensor. The finite engines agreed to 1e-3 at each tested rho, with one also
checked against a closed form. Transition excess costs agreed with independent
quadrature, with the difference decreasing under refinement.

The README and limits.md now explain two conventions:

  * **Undiscounted stationary models use the formal average-cost equations.**
    The dissertation's stationary verification fixes a stationary admissible profile, and
  ...
  * **Stationary costs report flow loss, including at positive discount.**
    ...
    field that identifies the cost convention.
...
The result contained an UNSUPPORTED check, but the CLI described it as a failed
check while reporting zero failures. The disabled check was corrected as
described above. The CLI also needed to distinguish unsupported checks from
failures that numerical settings might resolve.

The CLI now explains why a check is unsupported and whether settings can change
that. Added `--policy exploratory` to expose the policy selection available in
the library since 0.8; the CLI previously always used PUBLICATION.


The following documentation and interface changes complete the 0.8 API redesign.
The CLI rename from `transition --window` to `--T` is a breaking change and is
included in this release.

### Installed examples and introductory workflow

The README's first example used a repository-relative path unavailable in the wheel.
It also selected a model whose default window triggered WINDOW TOO SHORT.

Examples now install as `noisestate.examples`, mapped from the repository's
`examples/` directory. `ns.example(name)` returns an installed model's path,
and `ns.examples()` lists all seven. The README starts with
`ch1_two_player_finite`, which converges and passes publication checks in about
a second. The walkthrough was executed during development and is covered by a
regression test. The short-window example is retained in the diagnostics section.
Contributor instructions now use ./run-tests.
...
### Object display and introspection
...
## 0.8.0 (2026-09-10) — API and diagnostic contracts

Implemented the redesign in docs/design/api_spec.txt. Removed the names deprecated
in 0.7; this was a pre-publication API change.

### Statuses, policies, and assessments
...
The cell engine cannot receive acceptance under Policy.PUBLICATION.
Policy.EXPLORATORY remains available by explicit request.
...
### Result containers and model operations
...
## 0.7.0 (2026-09-09) — consistent API naming

Renamed fifteen public names to make conventions consistent across the 111-name API.
Old spellings remain available with deprecation warnings that identify their replacements.
They are scheduled for removal in 0.8.
...
## 0.6.9 (2026-09-09) — deprecated API removal and performance improvements
...
A validation plugin fingerprinted `to_dict(numeric=True)` for every model any engine
...
- **An atom operator's one nonzero block is asked for, not searched for.**  `atom_op` builds an
  ...
  Every shipped case is bit-identical with unchanged evaluation counts. There was no measurable
  performance improvement: the transition example took 5.38 s versus 5.41 s and peaked at the same
  366 MB. The allocator already recycled those pages, and this work was outside the fixed-point
  iteration. The change was retained to simplify the implementation.
...
- Triangle grid: a read point within round-off of a breakpoint is snapped to it before its panel is
  ...
  its panel by the sign of an ulp (found while testing shared quadrature paths across pieces).
...
- Age grid operators built from their block-Toeplitz core.  On uniform panels of width w the
  ...
  panel plus dense slabs for the non-uniform tail (reconstruction exact to 2e-15).  The
...
- Anderson memory 15 with damping 0.6 on the stationary engine (was 6 and 0.3): the Chapter 5 example
  ...
  cost of 0.01.
- Finite engine: delayed rows discretised exactly.  Two defects were found by comparison with the
...
- Fixed two additional cases after the exact-delay change: the breakpoints are closed under
...
- Stationary engine: delayed rows and lagged reads are discretised exactly.  A kernel read at a
  ...
  discretisation error.  Undelayed results are bit-for-bit unchanged.  Found by comparing against a discrete gradient derived directly from the one-agent problem.
