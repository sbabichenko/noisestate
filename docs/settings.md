...
| `foc_dense_max` | 8192 | spectral finite engine: unknowns nU nR N (the largest agent's) up to which the first-order-condition system is assembled (the operators applied to the identity) and LU-factored; above it it is solved by GMRES on the operators, preconditioned by time row (finite_free.FocSystem) |
...
| `resolution_tol` | 1e-6 | representation error above which a result is under-resolved (raise horizon.nodes) |
